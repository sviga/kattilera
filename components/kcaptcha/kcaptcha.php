<?php
//print_r($_SERVER);
//if (!eregi('index.php', $_SERVER['SCRIPT_NAME']))die();
...
class KCAPTCHA{
    var $root_path = '';
    var $kcaptcha_path = '/components/kcaptcha';
    var $tmpfolder="content/kc_tmp/"; //writeable by php
    var $maxold=1800;  // 0.5 h = 1800 s - после этого периода все темповые файлы удал€ютс€
    var $prefiks = 'cap_';
    var $gen_img_path = '';
	// generates keystring and image
	function makeKcaptcha(){

		require($this->root_path.$this->kcaptcha_path.'/kcaptcha_config.php');
		$fonts=array();
		$fontsdir_absolute=$this->root_path.$this->kcaptcha_path.'/'.$fontsdir;
		...
		$img_path = '/'.$this->tmpfolder.$this->prefiks.session_id().'_'.substr(mktime(), 5, 5).'.';

		if(function_exists("imagejpeg")){
			//header("Content-Type: image/jpeg");
			$this->gen_img_path = $img_path.'jpg';
			imagejpeg($img2, $this->root_path.$this->gen_img_path, $jpeg_quality);

		}else if(function_exists("imagegif")){
			//header("Content-Type: image/gif");
			$this->gen_img_path = $img_path.'gif';
			imagegif($img2, $this->root_path.$this->gen_img_path);
		}else if(function_exists("imagepng")){
			//header("Content-Type: image/x-png");
			$this->gen_img_path = $img_path.'png';
			imagepng($img2, $this->root_path.$this->gen_img_path);
		}
	}
